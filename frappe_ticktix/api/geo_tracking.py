...
def get_team_travel_summary(
    ...
    if employees:
        ...
        if isinstance(emp_list, list) and emp_list:
            emp_filter.append(["employee", "in", emp_list])
        ...
        emp_filter.append(["employee", "in", emp_ids])
    ...
    rows = frappe.db.sql(
        ...
        """.format(
            dept_clause=(
                "AND employee IN ({})".format(
                    ", ".join(["%s"] * len(emp_filter[-1][-1]))
                )
                if emp_filter and emp_filter[-1][1] == "in"
                else ""
            )
        ),
        (str(from_d), str(to_d)),
        as_dict=True,
    ) if not department and not employees else _team_summary_orm(emp_filter, from_d, to_d)
...
def _team_summary_orm(
    emp_filter: List[Any], from_d: date, to_d: date
) -> List[Dict[str, Any]]:
    """ORM-based fallback for department/employee-filtered team queries."""
...
def trigger_batch_processing(target_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    Args:
        target_date: Optional ISO-8601 date.  Defaults to today.
