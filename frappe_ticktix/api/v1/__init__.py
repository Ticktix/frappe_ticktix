...
def ticktix_login(redirect_to=None, tenant=None, mobile=None):
    ...
    # org_id / urn:zitadel:iam:... scoping is Zitadel-only - IdentityServer4
    # doesn't recognize those scope tokens, so this must not run when the
    # site is on the identityserver provider.
    ...
    if is_zitadel:
        ...
        configured_scope = ((config_manager.get_config_value('ticktix', {}) or {})
                             .get('oauth', {}).get('auth_params', {}).get('scope'))
        auth_params = {"response_type": "code", "scope": configured_scope or "openid profile email"}
