...
Responsibilities:
  - Read raw GPS records from ``Live GEO Tracking``
...
from .geo_utils import date_to_str

# ---------------------------------------------------------------------------
# Constants
# ---------------------------------------------------------------------------

_DOCTYPE_RAW = "Live GEO Tracking"
...
# NOTE — Partial-day consistency:
# When target_date is the current calendar day (not yesterday), GPS records
# are still being written by mobile devices in the field.  Any summary
# produced will be incomplete.  The nightly scheduler always processes
# *yesterday* (runs at 00:30) to avoid this.  Manual reprocessing of today
# via the API is permitted but callers should understand the data may be
# partial until end-of-day.
...
# ---------------------------------------------------------------------------
# Day-boundary helper (Fix: timezone-aware, Fix: sargable range support)
# ---------------------------------------------------------------------------

def _get_day_boundaries(date_str: str) -> Tuple[str, str]:
    """Return ``(start_dt, end_dt)`` strings for a full calendar day.

    Reads the site timezone from ``System Settings`` so that day boundaries
    are correct on servers in non-UTC zones.  Frappe stores naive datetime
    strings in the server's local (site) timezone, so the boundary is always
    midnight → 23:59:59 of that local date.

    Falls back to naive midnight strings when ``System Settings`` cannot be
    reached (e.g. during unit tests with a mocked Frappe).

    Using an explicit range (instead of ``DATE(col) = %s``) keeps the query
    **sargable** — the index on ``captured_at`` is fully utilised.
    """
    try:
        from zoneinfo import ZoneInfo  # Python 3.9+ / Frappe v15

        tz_name = (
            frappe.db.get_single_value("System Settings", "time_zone") or "UTC"
        )
        # Validate the tz name; ZoneInfo raises ZoneInfoNotFoundError if bad
        ZoneInfo(tz_name)

        d = date.fromisoformat(date_str)
        # Frappe stores naive datetimes in the configured site timezone,
        # so the day boundary is simply midnight–23:59:59 of that local date.
        start = datetime(d.year, d.month, d.day, 0, 0, 0)
        end = datetime(d.year, d.month, d.day, 23, 59, 59)
        return start.strftime("%Y-%m-%d %H:%M:%S"), end.strftime("%Y-%m-%d %H:%M:%S")

    except Exception:  # noqa: BLE001
        # Fallback: still correct for single-timezone deployments
...
def run_daily_travel_summary(target_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target_date:
        ...
        process_date = now_datetime().date() - timedelta(days=1)
    ...
    for employee in employees:
        try:
            # Guard: skip gracefully if the Employee record was deleted after
            # GPS data was already captured (prevents misleading FK errors).
            if not frappe.db.exists("Employee", employee):
                log.warning(
                    f"[GeoProcessing] SKIP {employee} on {date_str} "
                    f"— Employee record no longer exists"
                )
                skipped += 1
                continue
...
def _get_distinct_employees_for_date(date_str: str) -> List[str]:
    ...
    Uses a sargable range filter instead of ``DATE(device_date_time) = %s`` so
    the composite index on ``(employee, device_date_time)`` is fully utilised.
    """
    start_dt, end_dt = _get_day_boundaries(date_str)
    rows = frappe.db.sql(
        """
        SELECT DISTINCT employee
        FROM   `tabLive GEO Tracking`
        WHERE  device_date_time >= %s
          AND  device_date_time <= %s
...
def _fetch_raw_records(
    ...
    """Fetch GPS records for one employee on one date, sorted by device_date_time ASC.

    Uses explicit field list — never SELECT *.
    Day boundaries are resolved via :func:`_get_day_boundaries` to honour the
    site timezone configured in System Settings.
    """
    start_dt, end_dt = _get_day_boundaries(date_str)
    return frappe.db.get_all(
        ...
        filters=[
            ["employee", "=", employee],
            ["device_date_time", ">=", start_dt],
            ["device_date_time", "<=", end_dt],
        ],
        fields=["name", "employee", "device_date_time", "lat", "long",
...
def _get_existing_summary(
    ...
    if row:
        # Use .get() for compatibility with both frappe._dict (production)
        # and plain dict (unit-test mocks).
        return row.get("name"), row.get("status"), row.get("processed_at")
...
def _upsert_summary(
    ...
    # Wrap commit so a transient DB failure is logged and re-raised cleanly
    # rather than leaving the record in an invisible uncommitted state.
    try:
        frappe.db.commit()
    except Exception:  # noqa: BLE001
        frappe.db.rollback()
        _logger().error(
            f"[GeoProcessing] DB commit failed | employee={employee} "
