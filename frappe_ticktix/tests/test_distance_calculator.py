...
def _point(lat: float, lon: float, offset_minutes: int, accuracy: float = 5.0) -> dict:
    return {
        "latitude": lat,
        "longitude": lon,
        "captured_at": _ts(offset_minutes),
        "accuracy": accuracy,
    }
...
class TestNoiseFilters(unittest.TestCase):
    ...
    def test_filter_unrealistic_speed(self):
        ...
        base = datetime(2026, 1, 15, 8, 0, 0)
        points = [
            {"latitude": 3.0000, "longitude": 101.0000,
             "captured_at": base.strftime("%Y-%m-%d %H:%M:%S"), "accuracy": 5.0},
            {"latitude": 3.4500, "longitude": 101.0000,
             "captured_at": (base + timedelta(seconds=10)).strftime("%Y-%m-%d %H:%M:%S"),
             "accuracy": 5.0},
        ]
        m = calculate_travel_metrics(points, _SETTINGS)
        self.assertEqual(m.total_distance_km, 0.0)
    ...
    def test_filter_missing_timestamp(self):
        """Points with None or unparseable captured_at must be skipped."""
        points = [
            {"latitude": 3.0000, "longitude": 101.0000, "captured_at": None, "accuracy": 5},
            {"latitude": 3.0090, "longitude": 101.0000, "captured_at": "bad-date", "accuracy": 5},
    ...
    def test_filter_invalid_coordinates(self):
        ...
        points = [
            {"latitude": 999.0, "longitude": 101.0, "captured_at": _ts(0), "accuracy": 5},
