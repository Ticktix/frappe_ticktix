        ...
        if email_user:
            ...
            if user_sub:
                try:
                    ...
                    frappe.log_error(f"Failed to update social login mapping: {str(e)}", "JWT User Mapping")
            return email_user
...
def update_social_login_mapping(frappe_user, ticktix_user_id, username):
    ...
    try:
        ...
        frappe.log_error(f"Failed to update social login mapping for {frappe_user}: {str(e)}", "JWT User Mapping")
        raise
...
def auto_provision_jwt_user(jwt_payload):
    ...
    try:
        ...
        frappe.db.rollback()
        frappe.log_error(f"Failed to auto-provision JWT user {user_email}: {str(e)}", "JWT User Provisioning")
...
def assign_roles_from_jwt(user_doc, jwt_payload):
    ...
    for role in roles_to_assign:
        if frappe.db.exists("Role", role):
            ...
            frappe.log_error(f"Role {role} does not exist, skipping assignment for user {user_doc.email}", "JWT Role Assignment")
...
def sync_jwt_user_data():
    ...
    try:
        ...
        frappe.log_error(f"Failed to sync JWT user data for {current_user}: {str(e)}", "JWT User Sync")
