...
Validates JWT tokens via JWKS (public key), signature verification,
claims validation, and key caching.
"""
...
def validate_jwt_token_with_jwks(token, config=None):
    """
    Validate JWT token using the OIDC provider's JWKS endpoint for public key verification.
...
def get_public_key_from_jwks(config, key_id=None):
    """
    Fetch public key from the OIDC provider's JWKS endpoint.
...
def get_jwt_config():
    ...
    jwks_suffix = auth_config.get('ticktix_jwks_uri', '/oauth/v2/keys')
