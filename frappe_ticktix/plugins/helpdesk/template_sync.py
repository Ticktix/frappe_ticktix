...
class HelpdeskTemplateSyncManager:
    ...
    def sync_custom_fields(self):
        ...
        for cf in custom_fields:
            if cf.fieldname in existing_fields:
                skipped_count += 1
                frappe.log_error(
                    f"Field '{cf.fieldname}' already exists in template",
                    "Helpdesk Sync - Skipped"
                )
                continue
    ...
    def _add_field_to_template(self, custom_field):
        ...
        frappe.log_error(
            f"Added field '{custom_field.fieldname}' to template '{self.template_name}'",
            "Helpdesk Sync - Success"
        )
...
def auto_sync_on_custom_field_change(doc, method):
    ...
    if doc.dt == "HD Ticket" and doc.reqd == 1:
        ...
        frappe.log_error(
            f"Auto-sync triggered for field '{doc.fieldname}': {result.get('message')}",
            "Helpdesk Auto-Sync"
        )
